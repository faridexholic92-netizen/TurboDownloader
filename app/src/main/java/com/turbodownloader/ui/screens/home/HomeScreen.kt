    ...
    Scaffold(
        ...
        topBar = {
            LargeTopAppBar(
                title = {
                    Column {
                        Text("Turbo Downloader", fontWeight = FontWeight.Bold)
                        Text(
        ...
        LazyColumn(
            ...
            item {
                Row(
                    ...
                    StatCard(Icons.Default.Downloading, "Active", "$activeCount", Modifier.weight(1f),
                        listOf(MaterialTheme.colorScheme.primary.copy(alpha = 0.1f), MaterialTheme.colorScheme.primary.copy(alpha = 0.05f)))
                    StatCard(Icons.Default.DownloadDone, "Completed", "${completedDownloads.size}", Modifier.weight(1f),
                        listOf(MaterialTheme.colorScheme.tertiary.copy(alpha = 0.1f), MaterialTheme.colorScheme.tertiary.copy(alpha = 0.05f)))
                    val (free, _) = remember { viewModel.getStorageInfo() }
                    StatCard(Icons.Default.Storage, "Free", FileUtil.formatFileSize(free), Modifier.weight(1f),
                        listOf(MaterialTheme.colorScheme.secondary.copy(alpha = 0.1f), MaterialTheme.colorScheme.secondary.copy(alpha = 0.05f)))
                }
            }
                ...
                LazyRow(
                    ...
                    items(FileCategory.entries) { category ->
                        CategoryChip(
                            ...
                            onClick = { viewModel.selectCategory(if (selectedCategory == category) null else category) }
                        )
                    }
                }
            }
            ...
            items(items = filteredDownloads, key = { it.id }) { item ->
            ...
            if (filteredDownloads.isEmpty()) {
                item {
                    Column(
                        ...
                        Icon(Icons.Default.CloudDownload, null, Modifier.size(80.dp),
                            tint = MaterialTheme.colorScheme.onSurfaceVariant.copy(alpha = 0.4f))
                        Spacer(modifier = Modifier.height(16.dp))
                        Text("No downloads yet", style = MaterialTheme.typography.titleLarge,
                            color = MaterialTheme.colorScheme.onSurfaceVariant.copy(alpha = 0.6f))
                        Text("Tap + to start downloading", style = MaterialTheme.typography.bodyMedium,
    ...
    Card(
        ...
        Column(
            modifier = Modifier.fillMaxWidth()
                .then(if (gradient.isNotEmpty()) Modifier.background(Brush.verticalGradient(gradient)) else Modifier)
                .padding(12.dp),
            horizontalAlignment = Alignment.CenterHorizontally
        ) {
